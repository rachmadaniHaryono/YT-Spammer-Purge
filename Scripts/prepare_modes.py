...
def prepare_filter_mode_chars(scanMode, filterMode, config):
    ...
    if config["characters_to_filter"] != "ask":
        ...
        print(
            "          Note: Letters, numbers, and basic punctuation will not be included for safety purposes, even if you enter them."
        )
        print("          Example: 👋🔥✔️✨")
        input(
    ...
    while confirm == False:
        if (
            ...
            inputChars = make_char_set(
                config["characters_to_filter"],
                stripLettersNumbers=True,
                stripKeyboardSpecialChars=False,
            ...
            bypass = False
            print(
                f"\nWaiting for input Window. Press {F.MAGENTA}'Execute'{S.R} after entering valid characters to continue...",
                end="\r",
            )
            try:
                # Takes in user input of characters, returns 'set' of characters stripped of specified characters
                inputChars = take_input_gui(
                    mode="chars",
                    stripLettersNumbers=True,
                    stripKeyboardSpecialChars=False,
                    stripPunctuation=True,
                ...
                print(
                    "                                                                                          "
                )  # Clears the line because of \r on previous print
                print(
                    "\nError Code G-1: Something went wrong with the input, or you closed the window improperly."
                )
                print(
                    "If this keeps happening inexplicably, consider filing a bug report here: https://github.com/ThioJoe/YT-Spammer-Purge/issues"
                )
                input("Press Enter to exit...")
                sys.exit()

        print(
            f"     {whatToScanMsg} will be scanned for {F.MAGENTA}ANY{S.R} of the characters you entered in the previous window."
        )
    ...
    while validEntry == False:
        ...
        if validEntry == True:
            if config["strings_to_filter"] != "ask":
                ...
                print(
                    f"     {whatToScanMsg} will be scanned for {F.MAGENTA}ANY{S.R} of the following strings:"
                )
...
def prepare_filter_mode_regex(scanMode, filterMode, config):
    ...
    if config["regex_to_filter"] != "ask":
        ...
        print(
            f"Enter any {F.YELLOW}regex expression{S.R} to search within {whatToScanMsg}."
        )
    ...
    while validExpression == False:
        ...
        validationResults = validation.validate_regex(
            inputtedExpression
        ...
        if validExpression == True:
            ...
            print(
                f"     {F.RED}Error{S.R}: The expression appears to be {F.RED}invalid{S.R}!"
            )
            validConfigSetting = False
...
def prepare_filter_mode_ID(scanMode, config):
    processResult = (
        False,
        None,
    )  # Tuple, first element is status of validity of channel ID, second element is channel ID
    validConfigSetting = True
    while processResult[0] == False:
        if (
            ...
            bypass = False
            inputtedSpammerChannelID = input(
                f"Enter the {F.LIGHTRED_EX} Channel link(s) or ID(s){S.R} of the spammer (comma separated): "
            )
        ...
        if processResult[0] == True:
            inputtedSpammerChannelID = processResult[
                1
            ]  # After processing, if valid, inputtedSpammerChannelID is a list of channel IDs
        else:
            validConfigSetting = False
    print("\n")
    ...
    if any(auth.CURRENTUSER.id == i for i in inputtedSpammerChannelID):
        ...
        print(
            f"For safety purposes, this program always {F.YELLOW}ignores{S.R} your own comments."
        )
...
def prepare_filter_mode_non_ascii(scanMode, config):
    ...
    print(
        "~~~ This mode automatically searches for usernames that contain special characters (aka not letters/numbers) ~~~\n"
    )
    print(
        "Choose the sensitivity level of the filter. You will be shown examples after you choose."
    )
    print(
        f"   1. Allow {F.LIGHTMAGENTA_EX}Standard + Extended ASCII{S.R}:    Filter rare unicode & Emojis only"
    )
    print(
        f"   2. Allow {F.LIGHTMAGENTA_EX}Standard ASCII only{S.R}:  Also filter semi-common foreign characters"
    )
    print(
        f"   3. {F.LIGHTRED_EX}NUKE Mode (┘°□°)┘≈ ┴──┴ :    Allow ONLY numbers, letters, and spaces{S.R}"
    )
    print("")
    ...
    while confirmation == False:
        ...
        if selection == "1":
            print(
                f"Searches for {F.YELLOW}usernames with emojis, unicode symbols, and rare foreign characters{S.R} such as: ✔️ ☝️ 🡆 ▲ π Ɲ Œ"
            )
            ...
            print(
                f"Searches for {F.YELLOW}usernames with anything EXCEPT{S.R} the following: {F.YELLOW}Letters, numbers, punctuation, and common special characters{S.R} you can type with your keyboard like: % * & () + "
            )
            userChoice = choice("Choose this mode?", bypass)
            if userChoice == True:
            ...
            print(
                f"Searches for {F.YELLOW}usernames with anything EXCEPT letters, numbers, and spaces{S.R} - {B.RED}{F.WHITE} EXTREMELY LIKELY to cause collateral damage!{S.R} Recommended to just use to manually gather list of spammer IDs, then use a different mode to delete."
            )
            userChoice = choice("Choose this mode?", bypass)
            if userChoice == True:
    ...
    if selection == "1":
        ...
        autoModeName = (
            "NUKE Mode (┘°□°)┘≈ ┴──┴ - Allow only letters, numbers, and spaces"
        )
    ...
    if confirmation == True:
        ...
        input(
            "How did you get here? Something very strange went wrong. Press Enter to Exit..."
        )
        sys.exit()
...
def prepare_filter_mode_smart(scanMode, config, miscData, sensitive=False):
    ...
    spamDomainsList = miscData.spamLists[
        "spamDomainsList"
    ]  # List of domains from crowd sourced list
    # spamThreadsList = miscData.spamLists['spamThreadsList'] # List of filters associated with spam threads from crowd sourced list
    spamAccountsList = miscData.spamLists[
        "spamAccountsList"
    ]  # List of mentioned instagram/telegram scam accounts from crowd sourced list
    utf_16 = "utf-8"
    if config["filter_mode"] == "autosmart":
        ...
        print(
            f"~~~ This mode is a {F.LIGHTCYAN_EX}spammer's worst nightmare{S.R}. It automatically scans for multiple spammer techniques ~~~\n"
        )
        print(" > Extremely low (near 0%) false positives")
        print(" > Detects whatsapp scammers and '18+ spam' bots")
        print(
            " > Easily cuts through look-alike characters and obfuscations, including impersonating usernames"
        )
        if sensitive == False:
            print(
                f" > {F.LIGHTRED_EX}NOTE:{S.R} This mode prioritizes a {F.LIGHTGREEN_EX}VERY low false positive rate{S.R}, at the cost of occasionally missing some spammers.\n"
            )
        elif sensitive == True:
            print(
                f" > {F.LIGHTRED_EX}NOTE:{S.R} In sensitive mode, {F.LIGHTRED_EX}expect more false positives{S.R}. Recommended to run this AFTER regular Auto Smart Mode.\n"
            )
    ...
    textExactBlackWords_Raw = [
        b"Z*6BRAZ2)AV{~kJAa`hCbRcOUZe?X;Wn=",
        b"Z*6BRAZ2)AV{~kJAa`hCbRc<ebs%nKWn^V!",
        b"Z*6BRAZ2)AV{~kJAa`hCbRckLZ*Xj7AZ}%4WMyO",
        b"ZDnU+ZaN?$Xm50MWpW|",
        b"M`3zpIv^rJZDD$4WFi",
        b"X>%ZSa$$35EFf)pAY*TCbY*UIAZc>",
        b"X>%ZFVRB+&XJsrPZFwMLZ*FvDZge1Na{",
        b"Z*CwVX8",
    ]
    textUpLowBlackWords_Raw = [
        b"O<5pAPfk=tPE;UCQv",
        b"Ngz!@OGO}8L0KR|MO0KpQXoT5PE<usQ~",
        b"O<5pTNkm0YQy@W7MF",
        b"Qbj>TAWc~yP*P7uNlZl",
        b"Z*CwVM*",
    ]

    # General Settings
    unicodeCategoriesStrip = [
        "Mn",
        "Cc",
        "Cf",
        "Cs",
    ...
    for x in usernameBlackWords_Raw:
        usernameBlackWords.append(b64decode(x).decode(utf_16))
    for x in usernameNovidBlackWords_Raw:
        usernameNovidBlackWords.append(b64decode(x).decode(utf_16))
    for x in usernameObfuBlackWords_Raw:
        usernameObfuBlackWords.append(b64decode(x).decode(utf_16))
    for x in textExactBlackWords_Raw:
        textExactBlackWords.append(b64decode(x).decode(utf_16))
    for x in textUpLowBlackWords_Raw:
        textUpLowBlackWords.append(b64decode(x).decode(utf_16))

    # Type 1 Spammer Criteria
    minNumbersMatchCount = (
        6  # Choice of minimum number of matches from spamNums before considered spam
    )
    ...
    blackAdWords_Raw = [
        b"V`yb#YanfTAaHVTW@&5",
        b"Z*XO9AZ>XdaB^>EX>0",
        b"b7f^9ZFwMYa&Km7Yy",
        b"V`yb#YanfTAa-eFWp4",
        b"V`yb#YanoPZ)Rz1",
        b"V`yb#Yan)MWMyv",
        b"bYXBHZ*CxMc>",
        b"Z*CxMc_46UV{~<LWd",
    ...
    exactRedAdWords_Raw = [
        b"EiElAEiElAEiElAEiElAEiElAEiElAEiElAEiElAEiElAEiElAEiC",
        b"Wq4s@bZmJbcW7aBAZZ|OWo2Y#WB",
    ]
    redAdEmoji = b64decode(b"@Sl{P").decode(utf_16)
    yellowAdEmoji = b64decode(b"@Sl-|@Sm8N@Sm8C@Sl>4@Sl;H@Sly0").decode(utf_16)
    hrt = b64decode(
        b";+duJpOTpHpOTjFpOTmGpOTaCpOTsIpOTvJpOTyKpOT#LpQoYlpOT&MpO&QJouu%el9lkElAZ"
    ).decode(utf_16)

    # Create Type 2 Lists
    for x in blackAdWords_Raw:
        blackAdWords.append(b64decode(x).decode(utf_16))
    for x in redAdWords_Raw:
        redAdWords.append(b64decode(x).decode(utf_16))
    for x in yellowAdWords_Raw:
        yellowAdWords.append(b64decode(x).decode(utf_16))
    for x in exactRedAdWords_Raw:
        exactRedAdWords.append(b64decode(x).decode(utf_16))
    print("  Loading Filters  [===                           ]", end="\r")
    ...
    for x in threadWords_raw:
        threadWords.append(b64decode(x).decode(utf_16))
    for x in threadPhrases_raw:
        threadPhrases.append(b64decode(x).decode(utf_16))
    for x in monetWords_raw:
        monetWords.append(b64decode(x).decode(utf_16))
    for x in monetStrings_raw:
        monetStrings.append(b64decode(x).decode(utf_16))
    for x in salutations_raw:
        salutations.append(b64decode(x).decode(utf_16))
    for x in nakedNamePre_raw:
        nakedNamePre.append(b64decode(x).decode(utf_16))
    ...
    nakedNameRegex = re.compile(
        f"\\b({nakedNameString})\s+([a-zA-Z]+\.?)\s+([a-zA-Z]+)"
    )
    cashRegex = re.compile(
        r"^(\$|£|€)?(\d+|\d{1,3}(,\d{3})*)(\.\d+)?(\$|£|€|k| ?usd| ?eur| ?btc)?$"
    )
    ...
    bufferMatch, addBuffers = "\\*_~|`\\-\\._", re.escape(
        bufferChars
    )  # Add 'buffer' chars
    ...
    for word in usernameBlackWords:
        value = re.compile(
            confusable_regex(word.upper(), include_character_padding=True).replace(m, a)
        )
    ...
    for word in exactRedAdWords:
        value = re.compile(
            confusable_regex(word.upper(), include_character_padding=False)
        )
    ...
    for domain in spamDomainsList:
        spamListExpressionsList.append(
            confusable_regex(
                domain.upper().replace(".", "⚫"), include_character_padding=False
            ).replace("(?:⚫)", "(?:[^a-zA-Z0-9 ]{1,2})")
        )
    for account in spamAccountsList:
        spamListExpressionsList.append(
            confusable_regex(account.upper(), include_character_padding=True).replace(
                m, a
            )
        )
    ...
    print(
        "                                                                 "
    )  # Erases line that says "loading filters"
...
def recover_deleted_comments(config):
    ...
    print(
        '> Believe it or not, the YouTube API actually allows you to re-instate "deleted" comments.'
    )
    print(
        f"> This is {F.YELLOW}only possible if you have stored the comment IDs{S.R} of the deleted comments, \n   such as {F.YELLOW}having kept the log file{S.R} of that session."
    )
    print(
        "> If you don't have the comment IDs you can't recover the comments, and there is no way to find them. \n"
    )
    ...
    operations.delete_found_comments(
        commentsList=recoveryList,
        banChoice=False,
        deletionMode="published",
        recoveryMode=True,
    )
...
def delete_comment_list(config):
    ...
    if listChoice == "1":
        ...
        removalList, listFileNameBase = files.parse_comment_list(
            config, removal=True, returnFileName=True
        )
    ...
    if listChoice == "2":
        ...
        if len(existingSavesList) > 0:
            if len(existingSavesList) == 1:
                ...
                while valid == False:
                    ...
                    if (
                        saveChoice.isdigit()
                        and int(saveChoice) > 0
                        and int(saveChoice) <= len(existingSavesList)
                    ):
            ...
            progressFileNameWithPath = os.path.join(
                progressFileFolder, progressFileName
            )
            progressDict = files.read_dict_pickle_file(
                progressFileName, progressFileFolder
            )
            valid = True
            removalList = "Loaded"
        ...
        if removalList == "Loaded" or (
            len(remainingCommentsSet)
            + len(previousRemovedComments)
            + len(previousFailedComments)
        ) == len(removalList):
            pass
        else:
            print(
                f"{F.LIGHTRED_EX}Error:{S.R} The length of the comment list you loaded doesn't match the comment list you saved last time."
            )
            if (
                choice(
                    f"{F.YELLOW}Continue anyway?{S.R} (Will use previous save and ignore the file you just loaded)"
                )
                != True
            ):
                return "MainMenu"
        ...
        print(
            f" {F.LIGHTGREEN_EX}{prevRemovedNum} removed{S.R}  |  {F.YELLOW}{prevNotRemovedNum} not removed yet{S.R}  |  {F.LIGHTRED_EX}{prevFailedNum} failed to be removed{S.R}"
        )
        input("\n Press Enter to continue...")
        ...
        if len(previousFailedComments) > 0:
            print(
                f"{F.LIGHTRED_EX}NOTE:{S.R} During previous sessions, {F.LIGHTRED_EX}{len(previousFailedComments)} comments{S.R} failed to be deleted."
            )
            failChoice = choice(
    ...
    while validInput == False:
        ...
        try:
            ...
            if countChoice > 0 and countChoice <= quotaLimit:
                ...
                print(
                    f"Invalid input, must be 'all' or a whole number from 1 to {str(quotaLimit)}."
                )
        except:
    ...
    failedCommentsList = operations.delete_found_comments(
        commentsList=selectedRemovalList, banChoice=banChoice, deletionMode=removalMode
    )
    ...
    if partial == True or continued == True:
        ...
        if continued == True:
            ...
            progressDict = dict()
            progressDict[sessionNum] = {
                "removed": finalRemovedSet,
                "notRemoved": remainingCommentsSet,
                "failedCommentsList": failedCommentsList + previousFailedComments,
            }

    if (
        len(progressDict[sessionNum]["notRemoved"]) == 0
        and len(progressDict[sessionNum]["failedCommentsList"]) == 0
    ):
        if continued == True:
            print(
        ...
        # progressFileName = listFileNameBase + "_removal_progress.save"
        result = files.write_dict_pickle_file(
            progressDict, progressFileName, progressFileFolder, forceOverwrite=True
        )
        if result == True:
        ...
        print(
            f" {F.LIGHTGREEN_EX}{removed} removed{S.R}  |  {F.YELLOW}{notRemoved} not removed yet{S.R}  |  {F.LIGHTRED_EX}{failed} failed to be removed{S.R}"
        )
        print(
