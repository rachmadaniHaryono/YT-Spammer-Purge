...
def prepare_filter_mode_smart(scanMode, config, miscData, sensitive=False):
    ...
    nameRegex = re.compile(
        rf"\b{filter.salutationRegex}\s+([a-zA-Z]+\.?)\s+([a-zA-Z]+)"
    )
    nakedNameRegex = re.compile(
        rf"\b{filter.nakedNamePreRegex}\s+([a-zA-Z]+\.?)\s+([a-zA-Z]+)"
    )
    cashRegex = re.compile(
        r"((\$|£|€)((\d*( ?k))|((\d{3,})|(\d{1,3}((,|\.)\d{3}){1,}))))|(((\d*( ?k))|(((\d{3,})|(\d{1,3}(,|\.\d{3})))))(\$|£|€| ?u\.?s\.?d\.?| ?eur| ?btc| ?u.?s.? dollar))"
    )
    ...
    bufferMatch, addBuffers = "\\*_~|`\\-\\.", re.escape(
        bufferChars
    )  # Add 'buffer' chars
