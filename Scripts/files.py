...
def check_lists_update(spamListDict, silentCheck=False):
    ...
    if os.path.isdir(SpamListFolder):
        ...
        try:
            ...
            print(
                "Error: Could not create folder. Try creating a folder called 'spam_lists' to update the spam lists."
            )
    ...
    try:
        ...
        if response.status_code != 200:
            if response.status_code == 403:
                if silentCheck == False:
                    ...
                    print(
                        f"This means you have been {F.YELLOW}rate limited by github.com{S.R}. Please try again in a while.\n"
                    )
                    return False
                else:
                    return spamListDict
                ...
                if silentCheck == False:
                    print(
                        f"{B.RED}{F.WHITE}Error [U-3L]:{S.R} Got non 200 status code (got: {response.status_code}) when attempting to check for spam list update.\n"
                    )
                    print(
                        f"If this keeps happening, you may want to report the issue here: https://github.com/ThioJoe/YT-Spammer-Purge/issues"
                    )
                    if silentCheck == False:
                        return False
                else:
        ...
        if silentCheck == True:
            ...
            if "WinError 10013" in str(ox):
                print(
                    f"{B.RED}{F.WHITE}WinError 10013:{S.R} The OS blocked the connection to GitHub. Check your firewall settings.\n"
                )
                return False
    except:
        if silentCheck == True:
            ...
            print(
                "Error: Could not get latest release info from GitHub. Please try again later."
            )
            return False
            ...
            while True:
                try:
                    ...
                    if attempts <= 10:
                        ...
                        print(
                            f"This can happen if an antivirus takes a while to scan the file. You may need to manually extract the zip file."
                        )
...
def check_for_update(currentVersion, updateReleaseChannel, silentCheck=False):
    ...
    try:
        ...
        if response.status_code != 200:
            if response.status_code == 403:
                ...
                if silentCheck == False:
                    print(
                        f"{B.RED}{F.WHITE}Error [U-3]:{S.R} Got non 200 status code (got: {response.status_code}) when attempting to check for update.\n"
                    )
                    print(
                        f"If this keeps happening, you may want to report the issue here: https://github.com/ThioJoe/YT-Spammer-Purge/issues"
                    )
        ...
        if "WinError 10013" in str(ox):
            ...
            print(
                f"{B.RED}{F.WHITE}Unknown OSError{S.R} Error occurred while checking for updates\n"
            )
        return None
        ...
        if silentCheck == False:
            print(e + "\n")
            print(
                f"{B.RED}{F.WHITE}Error [Code U-1]:{S.R} Problem while checking for updates. See above error for more details.\n"
            )
            print(
            ...
            print(
                f"{B.RED}{F.WHITE}Error [Code U-1]:{S.R} Unknown problem while checking for updates. See above error for more details.\n"
            )
        return None
    ...
    if parse_version(latestVersion) > parse_version(currentVersion):
        ...
        if silentCheck == False:
            ...
            if isBeta == True:
                print(
                    f" {F.YELLOW}A new {F.LIGHTGREEN_EX}beta{F.YELLOW} version{S.R} is available! Visit {F.YELLOW}TJoe.io/latest{S.R} to see what's new."
                )
            else:
                print(
                    f" A {F.LIGHTGREEN_EX}new version{S.R} is available! Visit {F.YELLOW}TJoe.io/latest{S.R} to see what's new."
                )
            print(f"   > Current Version: {currentVersion}")
            print(f"   > Latest Version: {F.LIGHTGREEN_EX}{latestVersion}{S.R}")
            if isBeta == True:
                print(
                    "(To stop receiving beta releases, change the 'release_channel' setting in the config file)"
                )
            print(
                "------------------------------------------------------------------------------------------"
            )
            ...
            if userChoice == True:
                if sys.platform == "win32" or sys.platform == "win64":
                    ...
                    if j > 1:  # More than one exe file in release
                        print(
                            f"{F.YELLOW}Warning!{S.R} Multiple exe files found in release. You must be updating from the future when that was not anticipated."
                        )
                        print(
                            "You should instead manually download the latest version from: https://github.com/ThioJoe/YT-Spammer-Purge/releases"
                        )
                        print(
                            "You can try continuing anyway, but it might not be successful, or might download the wrong exe file."
                        )
                        input("\nPress enter to continue...")
                    elif j == 0:  # No exe file in release
                        print(
                            f"{F.LIGHTRED_EX}Warning!{S.R} No exe file found in release. You'll have to manually download the latest version from:"
                        )
                        print("https://github.com/ThioJoe/YT-Spammer-Purge/releases")
                        return False
                    if k == 0:  # No hash file in release
                        print(
                            f"{F.YELLOW}Warning!{S.R} No verification sha256 hash found in release. If download fails, you can manually download latest version here:"
                        )
                        print("https://github.com/ThioJoe/YT-Spammer-Purge/releases")
                        ...
                        print(
                            f"{F.YELLOW}Warning!{S.R} Too many or too few sha256 files found in release. If download fails, you should manually download latest version here:"
                        )
                        print("https://github.com/ThioJoe/YT-Spammer-Purge/releases")
                        input("\nPress Enter to try to continue...")
                    ...
                    if os.path.exists(downloadFileName):
                        ...
                        if confirm == True:
                            try:
                                ...
                                print(
                                    "The info above may help if it's a bug, which you can report here: https://github.com/ThioJoe/YT-Spammer-Purge/issues"
                                )
                    ...
                    if os.stat(downloadFileName).st_size == total_size_in_bytes:
                        ...
                        print(
                            "Something is wrong with the download on the remote end. You should manually download latest version here:"
                        )
                        print("https://github.com/ThioJoe/YT-Spammer-Purge/releases")
                    ...
                    if ignoreHash == False:
                        if (
                            ...
                            print(
                                "Or download the latest version manually from here: https://github.com/ThioJoe/YT-Spammer-Purge/releases"
                            )
                            return False
                    ...
                    if isBeta == False:
                        ...
                        print(
                            f" > And don't forget to report any problems you encounter here: {F.YELLOW}TJoe.io/bug-report{S.R}"
                        )
                    input("\nPress Enter to Exit...")
                    sys.exit()
        ...
        if silentCheck == False:
            print(
                f"\nYou have the {F.LIGHTGREEN_EX}latest{S.R} version: {F.LIGHTGREEN_EX}"
                + currentVersion
            )
        return False
    else:
        if silentCheck == False:
            print(
                "\nNo newer release available - Your Version: "
                + currentVersion
                + "  --  Latest Version: "
                + latestVersion
            )
...
def getRemoteFile(url, stream, silent=False, headers=None):
    try:
        ...
        if silent == False:
            print(e + "\n")
            print(
                f"{B.RED}{F.WHITE} Error {S.R} While Fetching Remote File or Resource: "
                + url
            )
...
def load_config_file(
    ...
    try:
        ...
        traceback.print_exc()
        print(
            f"{B.RED}{F.WHITE}Error Code: F-4{S.R} - Config file found, but there was a problem loading it! The info above may help if it's a bug."
        )
        print(
        ...
        if configDict["use_this_config"] == False:
            ...
            if configVersion != None:
                configDict = check_update_config_file(
                    configVersion, configDict, configFileName
                )
            ...
            print(
                "Error C-1: Invalid value in config file for setting 'use_this_config' - Must be 'True', 'False', or 'Ask'"
            )
            input("Press Enter to exit...")
            sys.exit()
...
def check_update_config_file(newVersion, existingConfig, configFileName):
    ...
    if configOutOfDate == True:
        ...
        print(
            f"  > Program will {F.LIGHTGREEN_EX}update your config{S.R} now, {F.LIGHTGREEN_EX}back up the old file{S.R}, and {F.LIGHTGREEN_EX}copy your settings over{S.R})"
        )
    ...
    if os.path.isfile(backupNameAndPath):
        print(
            "Existing backup config file found. Random number will be added to new backup file name."
        )
    ...
    try:
        ...
        return load_config_file(
            configVersion=None, skipConfigChoice=True, configFileName=configFileName
        )
    except:
        traceback.print_exc()
        print(
        ...
        print(
            "Something went wrong when copying your config settings. You'll have to manually copy them from backup."
        )
        input("\nPress Enter to exit...")
        sys.exit()
        ...
        if "spampurgeconfig" in file.lower() and file.lower() != "spampurgeconfig.ini":
            try:
                ...
                if "NoneType" in str(ax):
                    ...
                    print(
                        "Something went wrong when getting list of config files. Check your regex."
                    )
                    input("\nPress Enter to exit...")
                    sys.exit()
...
def choose_config_file(configDict, newestConfigVersion):
    ...
    if len(configFileList) > 0:
        ...
        if os.path.exists("SpamPurgeConfig.ini"):
            ...
            print(
                f"    {F.LIGHTCYAN_EX}N:{S.R} Use default settings, don't load any config"
            )
            print(
        ...
        for file in configFileList:
            ...
            configDescription = load_config_file(
                configFileName=file, skipConfigChoice=True
            )["this_config_description"]
        ...
        while valid == False:
            ...
            if configChoice.lower() == "y":
                ...
                chosenConfigDict = load_config_file(
                    skipConfigChoice=True, configFileName=configChoiceDict[configChoice]
                )
                chosenConfigDict = check_update_config_file(
                    newestConfigVersion,
                    chosenConfigDict,
                    configChoiceDict[configChoice],
                )
                return load_config_file(
                    skipConfigChoice=True, configFileName=configChoiceDict[configChoice]
                )
...
def create_config_file(
    updating=False, dontWarn=False, configFileName="SpamPurgeConfig.ini"
):
    ...
    if os.path.exists(configFileName):
        if updating == False and dontWarn == False:
            ...
            print(
                f"    1: {F.LIGHTRED_EX}Reset{S.R} main config ({F.LIGHTRED_EX}{configFileName}{S.R}) to fresh default config"
            )
            print(
                f"    2: {F.YELLOW}Create{S.R} another secondary config file (SpamPurgeConfig{F.YELLOW}{newConfigNum}{S.R}.ini)"
            )
            userChoice = input("\n Choose (1/2): ")
            ...
            if userChoice.lower() == "x":
                ...
                try:
                    ...
                    traceback.print_exc()
                    print(
                        "Error Code F-1: Problem deleting existing existing file! Check if it's gone. The info above may help if it's a bug."
                    )
                    print(
                        "If this keeps happening, you may want to report the issue here: https://github.com/ThioJoe/YT-Spammer-Purge/issues"
                    )
    ...
    try:
        ...
        traceback.print_exc()
        print(
            f"{B.RED}{F.WHITE}Error Code: F-2{S.R} - Problem reading default config file! The info above may help if it's a bug."
        )
        input("Press enter to Exit...")
        sys.exit()
    ...
    while success == False:
        try:
            ...
            traceback.print_exc()
            print(
                f"{B.RED}{F.WHITE}Error Code: F-3{S.R} Problem creating config file! The info above may help if it's a bug."
            )
            input("Press enter to Exit...")
            sys.exit()
    ...
    if os.path.exists(configFileName):
        ...
        try:
            ...
            if parser.get("info", "config_version"):
                ...
                print(
                    "Something might have gone wrong. Check if SpamPurgeConfig.ini file exists and has contents."
                )
                input("Press enter to Exit...")
                sys.exit()
        except:
            traceback.print_exc()
            print(
                "Something went wrong when checking the created file. Check if SpamPurgeConfig.ini exists and has text. The info above may help if it's a bug."
            )
            input("Press enter to Exit...")
            sys.exit()
...
def parse_comment_list(config, recovery=False, removal=False, returnFileName=False):
    ...
    while validFile == False and manuallyEnter == False:
        ...
        print(
            f"     > {F.BLACK}{B.LIGHTGREEN_EX} TIP: {S.R} You can just drag the file into this window instead of typing it"
        )
        print(
            f"{F.YELLOW}Or:{S.R} Just hit Enter to manually paste in the list of IDs next)"
        )
        ...
        if len(listFileName) > 0:
            ...
            if os.path.exists(listFileName):
                try:
                    ...
                    print(
                        f"{F.RED}Error Code F-5:{S.R} Log File was found but there was a problem reading it."
                    )
            else:
                print(
                ...
                print(
                    f"Enter '{F.YELLOW}Y{S.R}' to try again, or '{F.YELLOW}N{S.R}' to manually paste in the comment IDs."
                )
    ...
    if manuallyEnter == True:
        ...
        print(
            f'1. {F.YELLOW}Open the log file{S.R} and look for where it shows the list of {F.YELLOW}"IDs of Matched Comments".{S.R}'
        )
        print(
            f"2. {F.YELLOW}Copy that list{S.R}, and {F.YELLOW}paste it below{S.R} (In windows console try pasting by right clicking)."
        )
        print(
            "3. If not using a log file, instead enter the ID list in this format: FirstID, SecondID, ThirdID, ... \n"
        )
    ...
    if notValidCount > 0:
        print(
            f"{F.YELLOW}Possibly Invalid Comment IDs:{S.R} " + str(notValidList) + "\n"
        )
    ...
    if notValidCount == 0:
        ...
        print(
            f"{F.RED}Warning!{S.R} {str(validCount)} valid comment IDs loaded successfully, but {str(notValidCount)} may be invalid. See them above."
        )
...
def write_dict_pickle_file(
    ...
    while success == False:
        if os.path.isdir(relativeFolderPath):
            ...
            try:
                ...
                print(
                    f"Error: Could not create folder. Try creating the folder {relativeFolderPath} to continue."
                )
    ...
    while success == False:
        try:
            ...
            print(
                "Something went wrong when writing your pickle file. Did you open it or something?"
            )
        ...
        if os.path.exists(fileNameWithPath):
            ...
            while True and not failedAttemptCount > 2:
                try:
                    ...
                    print(
                        "Something went wrong when reading your pickle file. Is it in use? Try closing it."
                    )
                    input(
                        f"\nPress Enter to try loading file again: {fileNameWithPath}"
                    )
