...
def get_comments(
    ...
    nextPageToken=None,
    videosToScan=None,
):  # None are set as default if no parameters passed into function
    ...
    for item in results["items"]:
        ...
        if numReplies > 0 and len(limitedRepliesList) < numReplies:
            ...
            allVideoCommentsDict = get_replies(
                current,
                filtersDict,
                miscData,
                config,
                ...
                videosToScan,
                allVideoCommentsDict,
                parentCommentDict=parentCommentDict,
            )
        ...
        # If all the replies are in the limited list
            ...
            numReplies > 0 and len(limitedRepliesList) == numReplies
        ):  # limitedRepliesList can never be more than numReplies
            allVideoCommentsDict = get_replies(
                current,
                filtersDict,
                miscData,
            ...
            print_count_stats(
                current, miscData, videosToScan, final=False
            )  # Updates displayed stats if no replies
...
def get_replies(
    ...
    for reply in replies:
        ...
        try:
            ...
            if (
                filtersDict["filterMode"] == "Username"
                or filtersDict["filterMode"] == "AutoASCII"
                or filtersDict["filterMode"] == "AutoSmart"
                or filtersDict["filterMode"] == "NameAndText"
            ):
        ...
        check_against_filter(
            ...
            currentCommentDict,
            videoID,
            allThreadAuthorNames=allThreadAuthorNames,
        )
...
def check_spam_threads(
    ...
    threadWordsRegex = filtersDict["CustomCommentTextFilter"]["threadFiltersDict"][
        "threadWordsRegex"
    ]
    threadPhrasesRegex = filtersDict["CustomCommentTextFilter"]["threadFiltersDict"][
        "threadPhrasesRegex"
    ]
    monetWordsRegex = filtersDict["CustomCommentTextFilter"]["threadFiltersDict"][
        "monetWordsRegex"
    ]
    nameRegex = filtersDict["CustomCommentTextFilter"]["threadFiltersDict"]["nameRegex"]
    nakedNameRegex = filtersDict["CustomCommentTextFilter"]["threadFiltersDict"][
        "nakedNameRegex"
    ]
    ...
    if any(
        item in parentCommentDict["commentText"].lower()
        for item in miscData.spamLists["spamThreadsList"]
    ):
        add_spam(
            current,
            config,
            miscData,
            parentCommentDict,
            parentCommentDict["videoID"],
            matchReason="Spam Bot Thread",
        )
        return current
    ...
    for _, data in threadDict.items():
        if data["authorChannelID"] in threadAnalysisDict:
            threadAnalysisDict[data["authorChannelID"]] = (
                threadAnalysisDict[data["authorChannelID"]]
                + " "
                + re.sub(" +", " ", data["commentText"])
                .replace("\n", " ")
                .replace("\r", " ")
                .lower()
            )
    ...
    nameList, partialNameList, fullNameList = regexSearchNames(
        nameRegex, nameList, partialNameList, fullNameList
    )
    if not nameList:
        nameList, partialNameList, fullNameList = regexSearchNames(
            nakedNameRegex, nameList, partialNameList, fullNameList, naked=True
        )
        partialNameList = []
        fullNameList = []
    ...
    if (
        fullName in parentCommentDict["commentText"].lower()
        or name in parentCommentDict["commentText"].lower()
    ):
        fullNameCount += 1
        redCount += 1

    susRatio = susMentionCount / len(
        threadAnalysisList
    )  # Number of people, not replies
    ...
    if (
        redCount >= 1
        and yellowCount >= 5
        and (susRatio > 0.75 or (allNameCount >= 4 or fullNameCount >= 2))
    ):
        spam = True
    elif (
        redCount > 2
        and yellowCount >= 3
        and (susRatio > 0.70 or (allNameCount >= 4 or fullNameCount >= 2))
    ):
        spam = True
...
def add_spam(
    current, config, miscData, currentCommentDict, videoID, matchReason="Filter Match"
):
    ...
    dictToUse[commentID] = {
        "text": commentText,
        "textUnsanitized": commentTextRaw,
        "authorName": authorChannelName,
    ...
    current.vidIdDict[
        commentID
    ] = videoID  # Probably remove this later, but still being used for now
    ...
    for authorID, authorCommentsListofDicts in allCommentsDict.items():
        if authorID in matchedAuthorIDSet:
            for commentDict in authorCommentsListofDicts:
                scannedCount += 1
                print(
                    f" Finding all other comments by authors: [ {scannedCount/totalCommentsAmount*100:.2f}% ]".ljust(
                        40, " "
                    ),
                    end="\r",
                )
                if commentDict["commentID"] not in current.matchedCommentsDict:
                    add_spam(
                        ...
                        commentDict,
                        commentDict["videoID"],
                        matchReason="Also By Matched Author",
                    )
    print("".ljust(55, " "))
...
def check_duplicates(current, config, miscData, allVideoCommentsDict, videoID):
    ...
    for authorID, authorCommentsList in allVideoCommentsDict.items():
        ...
        if (
            auth.CURRENTUSER.id == authorID
            or miscData.channelOwnerID == authorID
            or authorID in miscData.resources["Whitelist"]["WhitelistContents"]
            or any(
            ...
            scannedCount += 1
            print(
                f" Analyzing For Duplicates: [ {scannedCount/authorCount*100:.2f}% ]   (Can be Disabled & Customized With Config File)".ljust(
                    75, " "
                ),
                end="\r",
            )
            ...
            if len(commentTextList) > 1:
                for i, x in enumerate(commentTextList):
                    ...
                    if len(x) >= minimum_duplicate_length or (
                        len(x) >= 6 and any(f".{domain}" in x for domain in domainList)
                    ):
...
def check_reposts(current, config, miscData, allVideoCommentsDict, videoID):
    ...
    for authorID, authorCommentsList in allVideoCommentsDict.items():
        for commentDict in authorCommentsList:
            if not commentDict[
                "parentAuthorChannelID"
            ]:  # Only bother checking top level comments
                flatCommentList.append(commentDict)

    flatCommentList.sort(
    ...
    for i, x in enumerate(
        flatCommentList[1:], start=1
    ):  # x is comment dictionary. Enumerate starting with second comment (1:), because nothing came before it. Use start=1 so i-1 refers to correct index in flatCommentList
        scrutinizedText = x["commentText"]
        scrutinizedAuthorID = x["authorChannelID"]
        if (
            scrutinizedAuthorID == auth.CURRENTUSER.id
            or scrutinizedAuthorID == miscData.channelOwnerID
            or scrutinizedAuthorID
            ...
            for j in range(
                0, i - 1
            ):  # Only need to check against comments that came before it, so have index less than current
                olderCommentText = flatCommentList[j]["commentText"]
                if (
                    len(scrutinizedText) >= minLength
                    and flatCommentList[j]["authorChannelID"] != scrutinizedAuthorID
                    and x["commentID"] not in current.matchedCommentsDict
                    and x["commentID"] not in current.duplicateCommentsDict
                ):
                    if (not fuzzy and scrutinizedText == olderCommentText) or (
                        fuzzy and ratio(scrutinizedText, olderCommentText) > levenshtein
                    ):
                        # List the indexes of the matched comments in the list
                        x["originalCommentID"] = flatCommentList[j]["commentID"]
                        add_spam(
                            current, config, miscData, x, videoID, matchReason="Repost"
                        )
                        break
        scannedCount += 1
        print(
            f" Analyzing For Stolen / Reposted Comments: [ {scannedCount/totalComments*100:.2f}% ]   (Can be Disabled & Customized With Config File)".ljust(
                75, " "
            ),
            end="\r",
        )
...
def check_against_filter(
    ...
    if (
        auth.CURRENTUSER.id != authorChannelID
        and miscData.channelOwnerID != authorChannelID
        and authorChannelID not in miscData.resources["Whitelist"]["WhitelistContents"]
    ):
        if "@" in commentText:
            # Logic to avoid false positives from replies to spammers
            if allThreadAuthorNames and (
                filtersDict["filterMode"] == "AutoSmart"
                or filtersDict["filterMode"] == "NameAndText"
            ):
        ...
        # Check Modes: Username
            ...
            if filtersDict["filterSubMode"] == "chars":
                ...
                if any(
                    x in filtersDict["CustomUsernameFilter"] for x in authorChannelName
                ):
                    add_spam(current, config, miscData, currentCommentDict, videoID)
            elif filtersDict["filterSubMode"] == "string":
                if utils.check_list_against_string(
                    listInput=filtersDict["CustomUsernameFilter"],
                    stringInput=authorChannelName,
                    caseSensitive=False,
                ):
                    add_spam(current, config, miscData, currentCommentDict, videoID)
        ...
        # Check Modes: Comment Text
            ...
            if filtersDict["filterSubMode"] == "chars":
                ...
                if any(
                    x in filtersDict["CustomCommentTextFilter"] for x in commentText
                ):
                    add_spam(current, config, miscData, currentCommentDict, videoID)
            elif filtersDict["filterSubMode"] == "string":
                if utils.check_list_against_string(
                    listInput=filtersDict["CustomCommentTextFilter"],
                    stringInput=commentText,
                    caseSensitive=False,
                ):
                    add_spam(current, config, miscData, currentCommentDict, videoID)
            elif filtersDict["filterSubMode"] == "regex":
        ...
        # Check Modes: Name and Text
            ...
            if filtersDict["filterSubMode"] == "chars":
                ...
                if utils.check_list_against_string(
                    ...
                    add_spam(current, config, miscData, currentCommentDict, videoID)
                elif utils.check_list_against_string(
                    listInput=filtersDict["CustomCommentTextFilter"],
                    stringInput=commentText,
                    caseSensitive=False,
                ):
        ...
        ## Also Check if reply author ID is same as parent comment author ID, if so, ignore (to account for users who reply to spammers)
            ...
            def findObf(expression, chars, stringToSearch, findall=True, phone=False):
                ...
                if not result:
                    ...
                    for match in result:
                        if match != "":
                            if not phone or (
                                phone and not bigNumCheckRegexCompiled.search(match)
                            ):
                                ...
                                while (
                                    ...
                                    match = match[:-1]
                                if any(
                                    char not in lowerChars for char in match
                                ) and any(
                                    char
                                    not in lowerChars.translate(
                                        ignoredConfusablesConverter
                                    )
                                    for char in match
                                ):
                                    return True
            ...
            def remove_unicode_categories(string):
                return "".join(
                    ...
                    not in smartFilter["unicodeCategoriesStrip"]
                )
            ...
            if index != -1:
                processedText = commentText.replace(
                    commentText[index : index + len(miscData.channelOwnerName)], ""
                )
            else:
            ...
            if authorChannelID == parentAuthorChannelID:
                ...
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif sensitive and findObf(
                phoneRegexCompiled, "0123456789+-() ", combinedString, phone=True
            ):
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif (
                not sensitive
                and compiledRegexDict["doubledSusWords"].search(
                    combinedStringNormalized
                )
                and findObf(
                    phoneRegexCompiled, "0123456789+-() ", combinedString, phone=True
                )
            ...
            #  add_spam(current, config, miscData, currentCommentDict, videoID)
                ...
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif config["detect_sub_challenge_spam"] and compiledRegexDict[
                "usernameNovidBlackWords"
            ].search(authorChannelName):
                add_spam(current, config, miscData, currentCommentDict, videoID)
                ...
                findObf(expressionPair[0], expressionPair[1], commentText)
                for expressionPair in compiledObfuRegexDict["textObfuBlackWords"]
            ):
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif preciseRegexDict["textExactBlackWords"].search(
                commentTextNormalized.lower()
            ):
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif preciseRegexDict["textUpLowBlackWords"].search(
                commentTextNormalized
            ) and not upLowTextSet.intersection(lowAlSet):
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif any(
                findObf(expressionPair[0], expressionPair[1], authorChannelName)
                ...
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif sensitive and re.search(
                smartFilter["usernameConfuseRegex"], authorChannelName
            ):
                add_spam(current, config, miscData, currentCommentDict, videoID)
            elif not sensitive and (
                findObf(
                    smartFilter["usernameConfuseRegex"],
                    miscData.channelOwnerName,
                    authorChannelName,
                )
                or authorChannelName == miscData.channelOwnerName
            ):
                add_spam(current, config, miscData, currentCommentDict, videoID)
            # Multi Criteria Tests
                ...
                if not sensitive and any(
                    emoji in commentTextNormalized for emoji in spamGenEmojiSet
                ):
                    yellowCount += 1

                if not sensitive and any(
                    emoji in authorChannelName for emoji in spamGenEmojiSet
                ):
                    redCount += 1
                ...
                if compiledRegexDict["usernameRedWords"].search(
                    authorChannelName.lower()
                ):
                    redCount += 1
...
def delete_found_comments(
    commentsList, banChoice, deletionMode, recoveryMode=False, skipCheck=False
):
    print("\n")
    ...
    def setStatus(commentIDs, failedComments):  # Does the actual deletion
        if deletionMode == "reportSpam":
            ...
            or deletionMode == "rejected"
            or deletionMode == "published"
        ):
            try:
                response = (
                    auth.YOUTUBE.comments()
            ...
            print(
                "Invalid deletion mode. This is definitely a bug, please report it here: https://github.com/ThioJoe/YT-Spammer-Purge/issues"
            )
    ...
    if total > 50:  # If more than 50 comments, break into chunks of 50
        remainder = (
            total % 50
        )  # Gets how many left over after dividing into chunks of 50
        numDivisions = int(
            (total - remainder) / 50
        )  # Gets how many full chunks of 50 there are
        for i in range(numDivisions):  # Loops through each full chunk of 50
            failedComments = setStatus(
                commentsList[i * 50 : i * 50 + 50], failedComments
            )
            deletedCounter += 50
            print_progress(deletedCounter, total, recoveryMode)
        if remainder > 0:
            failedComments = setStatus(
                commentsList[numDivisions * 50 : total], failedComments
            )  # Handles any leftover comments range after last full chunk
    ...
    if deletionMode == "reportSpam":
        print(
            f"{F.YELLOW}Comments Reported!{S.R} If no error messages were displayed, then everything was successful."
        )
        return failedComments
        ...
        print(
            "Comments Recovered! Will now verify each is back.                          \n"
        )
...
def check_deleted_comments(commentInput):
    ...
    print("                                                      ")
    print(
        "    (Note: You can disable deletion success checking in the config file, to save time and API quota)\n"
    )
    for commentID in commentList:
        try:
            ...
            print(
                "Verifying Deleted Comments: [" + str(j) + " / " + str(total) + "]",
                end="\r",
            )
            j += 1
        ...
        # If comment is found and possibly not deleted, print out video ID and comment ID
            ...
            if type(commentInput) == dict:
                print(
                    "Possible Issue Deleting Comment: "
                    + str(commentID)
                    + " |  Check Here: "
            ...
            if type(commentInput) == dict:
                print(
                    "Unhandled Exception While Deleting Comment: "
                    + str(commentID)
                    + " |  Check Here: "
                    + "https://www.youtube.com/watch?v="
                    + str(commentInput[commentID]["videoID"])
...
def check_recovered_comments(commentsList):
    ...
    if i == 0:
        ...
        print(
            "You can view them by using the links to them in the same log file you used."
        )
        ...
        print(
            "\n\nWarning: "
            + str(i)
            + " comments may have not been restored. See above list."
        )
        print(
            "Use the links to the comments from the log file you used, to verify if they are back or not."
        )
...
def exclude_authors(
    ...
    while valid == False:
        if "exclude" in inputtedString.lower() or "only" in inputtedString.lower():
            try:
                if (
                    "exclude" in inputtedString.lower()
                ):  # Account for if user is trying again
                    isolateExpression = (
                        r"(?<=exclude ).*"  # Matches everything after 'exclude'
                    )
                    result = str(
                        re.search(isolateExpression, inputtedString.lower()).group(0)
                    )
                elif "only" in inputtedString.lower():
                    isolateExpression = (
                        r"(?<=only ).*"  # Matches everything after 'exclude'
                    )
                    result = str(
                        re.search(isolateExpression, inputtedString.lower()).group(0)
                    )
        ...
        validInputExpression = (
            r"^[0-9,-]+$"  # Ensures only digits, commas, and dashes are used
        )
        if re.match(validInputExpression, result) == None:
            ...
            result = utils.expand_ranges(
                result
            )  # Expands ranges of numbers into a list of numbers
    ...
    for comment in commentIDExcludeSet:
        ...
        if comment in current.otherCommentsByMatchedAuthorsDict.keys():
            excludedCommentsDict[
                comment
            ] = current.otherCommentsByMatchedAuthorsDict.pop(comment)
    ...
    rtfFormattedExcludes += (
        f"(Values = Comment ID | Author ID | Author Name | Comment Text) \\line \n"
    )
    plaintextFormattedExcludes += f"\nComments Excluded From Deletion:\n"
    plaintextFormattedExcludes += (
        f"(Values = Comment ID | Author ID | Author Name | Comment Text)\n"
    )
    for commentID, meta in excludedCommentsDict.items():
        sanitizedText = (
            str(excludedCommentsDict[commentID]["text"])
            .replace("\n", " ")
            .replace("\r", " ")
        )
    ...
    for comment in current.matchedCommentsDict.keys():
        if comment in commentIDExcludeSet:
            print(
                f"{F.LIGHTRED_EX}FATAL ERROR{S.R}: Something went wrong while trying to exclude comments. No comments have been deleted."
            )
            print(
                f"You should {F.YELLOW}DEFINITELY{S.R} report this bug here: https://github.com/ThioJoe/YT-Spammer-Purge/issues"
            )
    ...
    return (
        ...
        rtfFormattedExcludes,
        plaintextFormattedExcludes,
    )  # May use excludedCommentsDict later for printing them to log file
...
def get_recent_videos(current, channel_id, numVideosTotal):
    def get_block_of_videos(nextPageToken, j, k, numVideosBlock=50):
        ...
        for item in result["items"]:
            ...
            if str(commentCount) == "0":
                print(
                    f"{B.YELLOW}{F.BLACK} Skipping {S.R} {F.LIGHTRED_EX}Video with no comments:{S.R} "
                    + str(item["snippet"]["title"])
                )
                k += 1
                continue
    ...
    j, k = (
        0,
        0,
    )  # i = number of videos added to list, k = number of videos checked (different only if one video skipped because no comments)
    if numVideosTotal <= 5:
        ...
        while (
            nextPageToken != "End"
            and k < numVideosTotal
            and str(abortCheck) != "MainMenu"
        ):
            print(
                "Retrieved "
                + str(len(recentVideos))
                + "/"
                + str(numVideosTotal)
                + " videos.",
                end="\r",
...
def print_count_stats(current, miscData, videosToScan, final):
    ...
    if videosToScan and miscData.totalCommentCount > 0:
        ...
        progress = (
            f"Total: [{str(totalScanned)}/{str(totalComments)}] ({percent:.0f}%) ".ljust(
                27, " "
            )
            + "|"
        )  # Formats percentage to 0 decimal places
    else:
        progress = ""
    ...
    if final == True:
        print(
            f" {progress} Comments Scanned: {F.YELLOW}{comScanned}{S.R} | Replies Scanned: {F.YELLOW}{repScanned}{S.R} | Matches Found So Far: {F.LIGHTRED_EX}{matchCount}{S.R}\n"
        )
    else:
        print(
            f" {progress} Comments Scanned: {F.YELLOW}{comScanned}{S.R} | Replies Scanned: {F.YELLOW}{repScanned}{S.R} | Matches Found So Far: {F.LIGHTRED_EX}{matchCount}{S.R}",
