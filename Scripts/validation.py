...
def validate_video_id(
    video_url_or_id, silent=False, pass_exception=False, basicCheck=False
):
        ...
        try:
            ...
            if possibleVideoID == result["items"][0]["id"]:
                ...
                try:
                    ...
                    print(
                        f"                    Bug Report Link: {F.YELLOW}TJoe.io/bug-report{S.R}"
                    )
                ...
                if silent == False:
                    print(
                        "Something very odd happened. YouTube returned a video ID, but it is not equal to what was queried!"
                    )
                return False, None, None, None, None
...
def validate_channel_id(inputted_channel):
    ...
    if "/channel/" in inputted_channel:
        ...
        if "/c/" in inputted_channel:
            startIndex = (
                inputted_channel.rindex("/c/") + 3
            )  # Start index at at character after /c/
        ...
        if startIndex < endIndex and endIndex <= len(inputted_channel):
            ...
            response = (
                auth.YOUTUBE.search()
                .list(part="snippet", q=customURL, maxResults=1)
                .execute()
            )
    ...
    # Handle legacy style custom URL (no /c/ for custom URL)
        ...
        if startIndex < endIndex and endIndex <= len(inputted_channel):
            ...
            if re.match(r"[0-9A-Za-z_-]{10}[048AEIMQUYcgkosw]", customURL):
                print(
                    f"{F.LIGHTRED_EX}Invalid Channel ID / Link!{S.R} Did you enter a video ID / link by mistake?"
                )
                return False, None, None
    ...
    if len(isolatedChannelID) == 24 and isolatedChannelID[0:2] == "UC":
        response = (
            auth.YOUTUBE.channels().list(part="snippet", id=isolatedChannelID).execute()
        )
        if response["items"]:
            ...
            print(
                f"{F.LIGHTRED}Error{S.R}: Unable to Get Channel Title. Please check the channel ID."
            )
            return False, None, None
...
def validate_config_settings(config):
    ...
    def print_int_fail(setting, value):
        ...
        print(
            "It should probably be an integer. Check the possible values listed in the config file for that setting."
        )
    ...
    def simple_settings_check(setting, value):
        if setting in validSettingsDict:
            ...
            # For settings that accept more than one value, check each
                ...
                try:
                    ...
                    for settingValue in settingList:
                        if settingValue not in validSettingsDict[setting]:
                            ...
                            print(
                                f"It looks like you tried to enter a list. Check if that setting accepts multiple values or if you entered an invalid value."
                            )
                            print_quit_and_report()
                    return True
                except:
                    ...
                    print(
                        "It looks like you tried to enter a list. Check if that setting accepts multiple values or if you entered an invalid value."
                    )
                    print_quit_and_report()
                ...
                print(
                    "Check the config file to see valid possible values for that setting."
                )
                print_quit_and_report()
        else:
            return None
    ...
    def validate_videos_to_scan(value, *args):
        if value == "ask":
            ...
            try:
                ...
                print(
                    "Make sure it is either a single video ID / Link, or a comma separate list of them!"
                )
                print_quit_and_report()
            if len(videoList) > 0:
    ...
    def validate_channel_to_scan(value, *args):
        if value == "ask" or value == "mine":
            ...
            if result == False:
                ...
                print(
                    "Make sure it is either a single channel ID or channel link.  If it's a link, try using the channel ID instead."
                )
                print_quit_and_report()
            else:
                return True
    ...
    def validate_channel_ids_to_filter(value, *args):
        if value == "ask":
            ...
            try:
                ...
                print(
                    "Make sure it is either a single channel ID / Link, or a comma separate list of them!"
                )
                print_quit_and_report()
            for channel in channelList:
                if len(channel) != 24 or channel[0:2] != "UC":
                    ...
                    print(
                        "A channel ID must be 24 charactres long and begin with 'UC'!"
                    )
                    print_quit_and_report()
            return True
    ...
    def validate_chars(value, *args):
        ...
        if result:
            ...
            print(
                "For this mode, numbers, letters, and punctuation are removed. But there were no characters left to search!"
            )
            print_quit_and_report()
    ...
    def validate_strings(value, *args):
        ...
        try:
            ...
            if len(result) > 0:
                ...
                print(
                    "The list appears empty! Make sure it is either a single string, or a comma separate list of them!"
                )
                print_quit_and_report()
        except:
            print(
            ...
            print(
                "Make sure it is either a single string, or a comma separate list of them!"
            )
            print_quit_and_report()
    ...
    for settingName, settingValue in config.items():
        ...
        # Check simple value settings
            ...
            if result == True:
                ...
                print(
                    f"If you didn't add or change this setting in the config file, a validation check was probably forgotten to be created!"
                )
    ...
    for settingName in allSettingsDict:
        if settingName not in list(config.keys()):
            ...
            print(
                " > Did you remove it or are you using an old config file? (It should have auto-updated)"
            )
