...
def print_comments(
    ...
    j, commentsContents = print_prepared_comments(
        ...
        commentsContents,
        scanVideoID,
        list(current.matchedCommentsDict.keys()),
        j,
        loggingEnabled,
        scanMode,
        logMode,
        doWritePrint,
        matchReason="Filter Match",
    )
    ...
    if current.spamThreadsDict:
        j, commentsContents = print_prepared_comments(
            ...
            commentsContents,
            scanVideoID,
            list(current.spamThreadsDict.keys()),
            j,
            loggingEnabled,
            scanMode,
            logMode,
    ...
    if current.otherCommentsByMatchedAuthorsDict:
        j, commentsContents = print_prepared_comments(
            ...
            commentsContents,
            scanVideoID,
            list(current.otherCommentsByMatchedAuthorsDict.keys()),
            j,
            loggingEnabled,
            scanMode,
            logMode,
    ...
    if current.duplicateCommentsDict:
        j, commentsContents = print_prepared_comments(
            ...
            commentsContents,
            scanVideoID,
            list(current.duplicateCommentsDict.keys()),
            j,
            loggingEnabled,
            scanMode,
            logMode,
    ...
    if current.repostedCommentsDict:
        j, commentsContents = print_prepared_comments(
            ...
            commentsContents,
            scanVideoID,
            list(current.repostedCommentsDict.keys()),
            j,
            loggingEnabled,
            scanMode,
            logMode,
    ...
    if current.spamThreadsDict and current.vidTitleDict:
        ...
        if any(
            word in str(list(current.vidTitleDict.values())).lower()
    ...
    def print_and_write(value, writeValues, printValues):
        ...
        if doWritePrint:
            printValues = (
                printValues + indexString + countString + f"{str(nameAndText)}\n"
            )
        # After making print values, remove the ANSI escape / color codes used, so they won't be written to file
        indexString = indexString.replace("\u001b[32m", "").replace("\u001b[0m", "")
        countString = countString.replace("\u001b[32m", "").replace("\u001b[0m", "")
        nameAndText = nameAndText.replace("\u001b[32m", "").replace("\u001b[0m", "")
        ...
        if loggingEnabled == True and logMode == "rtf":
            writeValues = (
                ...
                + indexString
                + countString
                + f"{str(authorID)} | {make_rtf_compatible(str(nameAndText))} \\line \n"
            )
            ...
            writeValues = (
                ...
                + indexString
                + countString
                + f"{str(authorID)} | {str(nameAndText)}\n"
            )
    ...
    if doWritePrint:
        print(
            f"{F.LIGHTMAGENTA_EX}============================ Match Samples: One comment per matched-comment author ============================{S.R}"
        )
        if possibleFalsePositive:
            print(
                f"{F.GREEN}======= {B.GREEN}{F.BLACK} NOTE: {S.R}{F.GREEN} Possible false positives marked with * and highlighted in green. Check them extra well! ======={S.R}"
            )
    for value in current.matchSamplesDict.values():
        if (
            value["matchReason"] != "Duplicate"
            and value["matchReason"] != "Spam Bot Thread"
            and value["matchReason"] != "Repost"
        ):
            valuesPreparedToWrite, valuesPreparedToPrint = print_and_write(
                value, valuesPreparedToWrite, valuesPreparedToPrint
            )
        # If there are duplicates, save those to print later, but get ready by calculating some duplicate info
            ...
            if config["fuzzy_stolen_comment_detection"] == True:
                repostSimilarity = (
                    str(round(float(config["levenshtein_distance"]) * 100)) + "%"
                )
            else:
    ...
    if hasSpamThreads == True:
        if doWritePrint:
            print(
                f"{S.BRIGHT}{F.MAGENTA}============================ Match Samples: Spam Bot Threads ============================{S.R}"
            )
            if spamThreadNotice == True:
                print(
                    f"{F.YELLOW}{F.BLACK}{B.YELLOW} NOTE: {S.R}{F.YELLOW} If video is about investing/crypto, inspect these extra well for false positives{S.R}"
                )
                print(
                    "-----------------------------------------------------------------------------------------"
                )
    for value in current.matchSamplesDict.values():
        if value["matchReason"] == "Spam Bot Thread":
            (
                ...
                value, spamThreadValuesPreparedToWrite, spamThreadValuesPreparedToPrint
            )
    if doWritePrint:
    ...
    if hasDuplicates == True:
        if doWritePrint:
            print(
                f"{F.LIGHTMAGENTA_EX}------------------------- {S.BRIGHT}{F.WHITE}{B.BLUE} Non-Matched {S.R}{F.LIGHTCYAN_EX} Commenters, But Who Wrote Many Similar Comments{F.LIGHTMAGENTA_EX} -------------------------{S.R}"
            )
            print(
                f"{F.MAGENTA}---------------------------- ( {F.LIGHTBLUE_EX}Similarity Threshold: {similarity}  |  Minimum Duplicates: {minDupes}{F.MAGENTA} ) ----------------------------{S.R}"
            )
    for value in current.matchSamplesDict.values():
        if value["matchReason"] == "Duplicate":
            duplicateValuesToWrite, duplicateValuesToPrint = print_and_write(
                value, duplicateValuesToWrite, duplicateValuesToPrint
            )
    if doWritePrint:
    ...
    if hasReposts == True:
        print(
            f"{F.LIGHTMAGENTA_EX}------------------------- {S.BRIGHT}{F.WHITE}{B.BLUE} Non-Matched {S.R}{F.LIGHTCYAN_EX} Commenters, But Who Reposted a Previous Comment{F.LIGHTMAGENTA_EX} -------------------------{S.R}"
        )
        print(
            f"{F.MAGENTA}---------------------------- ( {F.LIGHTBLUE_EX}Similarity Threshold: {repostSimilarity}  |  Minimum Length: {minLength}{F.MAGENTA} ) ------------------------------{S.R}"
        )
    for value in current.matchSamplesDict.values():
        if value["matchReason"] == "Repost":
            repostValuesToWrite, repostValuesToPrint = print_and_write(
                value, repostValuesToWrite, repostValuesToPrint
            )
    ...
    if loggingEnabled == True:
        ...
        if logMode == "rtf":
            if possibleFalsePositive:
                ...
                addLine = "\\line\\line \n"
            matchSamplesContent = (
                f"==================== Match Samples: One comment per matched-comment author ==================== {addLine}"
                + valuesPreparedToWrite
            )
            ...
            if hasDuplicates == True:
                ...
                duplicateSamplesContent += (
                    f"---------------------- ( Similarity Threshold: {similarity}  |  Minimum Duplicates: {minDupes} ) ---------------------- \\line\\line \n"
                    + duplicateValuesToWrite
                )
                if doWritePrint:
            ...
            if hasReposts == True:
                ...
                repostSamplesContent += (
                    f"------------------------ ( Similarity Threshold: {repostSimilarity}  |  Minimum Length: {minLength} ) ------------------------ \\line\\line \n"
                    + repostValuesToWrite
                )
                if doWritePrint:
            ...
            matchSamplesContent = (
                f"==================== Match Samples: One comment per matched-comment author ====================\n{addLine}"
                + valuesPreparedToWrite
            )
            if doWritePrint:
            ...
            if hasDuplicates == True:
                ...
                duplicateSamplesContent += (
                    f"---------------------- ( Similarity Threshold: {similarity}  |  Minimum Duplicates: {minDupes} ) ----------------------\n"
                    + duplicateValuesToWrite
                )
                if doWritePrint:
        ...
        if hasReposts == True:
            ...
            repostSamplesContent += (
                f"------------------------ ( Similarity Threshold: {repostSimilarity}  |  Minimum Length: {minLength} ) ------------------------\n"
                + repostValuesToWrite
            )
            if doWritePrint:
    ...
    if doWritePrint:
        print(
            f"{F.LIGHTMAGENTA_EX}==================== (See log file for channel IDs of matched authors above) ===================={S.R}"
        )
...
def print_prepared_comments(
    ...
    for comment in comments:
        ...
        if (
            matchReason != "Also By Matched Author"
            and author_id_local not in current.matchSamplesDict.keys()
        ):
        ...
        if scanMode == "communityPost" or scanMode == "recentCommunityPosts":
            directLink = (
                "https://www.youtube.com/post/" + videoID + "?lc=" + comment_id_local
            )
        else:
            directLink = (
                "https://www.youtube.com/watch?v=" + videoID + "&lc=" + comment_id_local
            )
        if isRepost:
            repostLink = (
        ...
        if doWritePrint:
            ...
            print(
                f"     > Author Channel ID: {F.LIGHTBLUE_EX}"
                + author_id_local
                + f"{S.R}"
            )
            print(
        ...
        if loggingEnabled == True:
            ...
            if isRepost:
                if logMode == "rtf":
                    ...
                    originalCommentInfoLine = (
                        "         >> Original Comment ID: " + repostLink + "\n"
                    )
            else:
                originalCommentInfoLine = ""
...
def make_rtf_compatible(text):
    try:
        ...
        intermediate = "".join(
            char
            for char in text
            if unicode_category(char) not in ["Mn", "Cc", "Cf", "Cs", "Co", "Cn"]
        )
        try:
    ...
    if firstWrite == True or fullWrite == True:
        ...
        if not os.path.isdir(logFolderPath):
            try:
                ...
                print(
                    f"{F.LIGHTRED_EX}Error:{S.R} Could not create desired directory for log files. Will place them in current directory."
                )
        ...
        while success == False:
            try:
                ...
                with open(fileName, "w", encoding="utf-8") as file:
                    ...
                    file.write(
                        r"{\colortbl;\red0\green0\blue0;\red255\green0\blue0;\red0\green255\blue0;\red0\green0\blue255;\red143\green81\blue0;\red102\green0\blue214;}"
                        + "\n\n"
                    )
                    if fullWrite == True:
                        file.write(newText)
            ...
            try:
                ...
                with open(fileName, "r+", encoding="utf-8") as file:
                    ...
                    for (
                        ...
                        newText
                    ):  # write new lines. If any brackets, add escape backslash
                        line.replace("}", "\\}")
                        line.replace("{", "\\{")
                        file.write(line)
                    file.write(
    ...
    if config["json_log_all_comments"] == True:
        ...
        for authorID in allCommentsDict:
            for i, comment in enumerate(allCommentsDict[authorID]):
                if (
                    comment["commentID"] in dictionaryToWrite["Comments"]
                ):  # If it's in the dictionary with spam comments
                    commentID = comment["commentID"]
                    allCommentsDict[authorID][i]["isSpam"] = "True"
                    allCommentsDict[authorID][i]["matchReason"] = dictionaryToWrite[
                        "Comments"
                    ][commentID]["matchReason"]
        ...
        try:
            ...
            with open(fileName, "w", encoding=jsonEncoding) as file:
                if config["json_log_all_comments"] == True:
                    ...
                    file.write(
                        json.dumps(dictionaryToWrite, indent=4, ensure_ascii=False)
                    )
                    file.close()
            success = True
        except PermissionError:
...
def get_extra_json_data(channelIDs, jsonSettingsDict):
    ...
    if jsonSettingsDict["json_profile_picture"] != False:
        ...
        if resolution not in possibleResolutions:
            print(
                f"{B.RED}{F.BLACK}Invalid Resolution!{S.R} Defaulting to 'default' (smallest)"
            )
            resolution = "default"
    ...
    def fetch_data(channelIdGroup):
        try:
            response = (
                ...
                .list(
                    part="snippet,statistics", id=channelIdGroup, fields=fieldsToFetch
                )
                .execute()
            )
            if response["items"]:
    ...
    response = (
        auth.YOUTUBE.channels()
        .list(part="snippet,statistics", id=channelOwnerID, fields=fieldsToFetch)
        .execute()
    )
    if response["items"]:
        tempDict = {}
...
def download_profile_pictures(pictureUrlsDict, jsonSettingsDict):
    ...
    if not os.path.isdir(imageFolderPath):
        try:
            ...
            print(
                f"{F.LIGHTRED_EX}Error:{S.R} Unable to create image folder. Try creating a folder called 'ProfileImages' in the log file folder."
            )
            return False, None
...
def add_sample(current, authorID, authorNameRaw, commentText, matchReason):
    def remove_unicode_categories(string):
        ...
        return "".join(
            char for char in string if unicode_category(char) not in unicodeStrip
        )
...
def sort_samples(current):
    ...
    sortedTupleList = list(
        reversed(sorted(dictToSort.items(), key=lambda item: item[1][sortBy]))
    )
    ...
    for author, dictionary in newDict.items():
        ...
        if dictionary["possibleFalsePositive"] == True:
            ...
            newDict[author][
                "cString"
            ] = f"{F.GREEN}{dictionary['cString']}{S.R}"  # cString is 'count string'
            newDict[author][
                "nameAndText"
            ] = f"{F.GREEN}{dictionary['nameAndText']}{S.R}"
        else:
...
def prepare_logFile_settings(
    ...
    if config["log_path"]:
        if (
            config["log_path"] == "default"
        ):  # For backwards compatibility, can remove later on
            logPath = defaultLogPath
        else:
            logPath = config["log_path"]
        current.logFileName = os.path.normpath(logPath + "/" + fileName)
        print(
            f"Log file will be located at {F.YELLOW}" + current.logFileName + f"{S.R}\n"
        )
        if jsonLogging == True:
            jsonLogFileName = os.path.normpath(logPath + "/" + jsonLogFileName)
            jsonSettingsDict["jsonLogFileName"] = jsonLogFileName
            print(
                f"JSON log file will be located at {F.YELLOW}"
                + jsonLogFileName
                + f"{S.R}\n"
            )
    else:
...
def write_log_heading(
    current, logMode, filtersDict, afterExclude=False, combinedCommentsDict=None
):
    ...
    if current.scannedThingsList:
        write_func(
            current.logFileName,
            "Video(s) or Post(s) Scanned: " + ", ".join(current.scannedThingsList),
            logMode,
            2,
        )
    ...
    if filterMode == "ID":
        write_func(
            current.logFileName,
            "Channel IDs of spammer searched: " + ", ".join(inputtedSpammerChannelID),
            logMode,
            2,
        )
        ...
        write_func(
            current.logFileName,
            "Searched in Usernames: " + ", ".join(inputtedUsernameFilter),
            logMode,
            2,
        )
        ...
        write_func(
            current.logFileName,
            "Searched in Comment Text: " + ", ".join(inputtedCommentTextFilter),
            logMode,
            2,
        )
        ...
        write_func(
            current.logFileName,
            "Auto-ASCII Username Search Mode: " + str(filterSettings[1]),
            logMode,
            2,
        )
    elif filterMode == "AutoSmart":
        write_func(
            current.logFileName, "Automatic Search Mode: Smart Mode ", logMode, 2
        )
    elif filterMode == "SensitiveSmart":
        write_func(
            current.logFileName, "Automatic Search Mode: Sensitive Smart ", logMode, 2
        )

    # Write number of comments for each type
    write_func(
        current.logFileName,
        "Number of Matched Comments Found: " + str(len(current.matchedCommentsDict)),
        logMode,
        2,
    )
    write_func(
        current.logFileName,
        "Number of Spam Bot Threads Found: " + str(len(current.spamThreadsDict)),
        logMode,
        2,
    )
    write_func(
        current.logFileName,
        "Number of Non-Matched, but Duplicate Comments Found: "
        + str(len(current.duplicateCommentsDict)),
        logMode,
        2,
    )
    write_func(
        current.logFileName,
        "Number of Non-Matched, but Stolen / Reposted Comments Found: "
        + str(len(current.repostedCommentsDict)),
        logMode,
        2,
    )
    ...
    write_func(
        current.logFileName,
        f"{commentListLabel}{excludeString}: \n[ {', '.join(combinedCommentsDict)} ] ",
        logMode,
        3,
    )
...
def write_log_completion_summary(
    current,
    exclude,
    logMode,
    banChoice,
    deletionModeFriendlyName,
    removeOtherAuthorComments,
):
    if logMode == "rtf":
        write_rtf(
        ...
        write_rtf(
            current.logFileName,
            "\n\n\\line\\line Action Taken on Comments: "
            + str(deletionModeFriendlyName)
            + " \\line\\line \n\n",
        )
        write_rtf(
            current.logFileName,
            "\n\n\\line\\line Also Retrieved All Other Comments by Matched Authors: "
            + str(removeOtherAuthorComments)
            + " \\line\\line \n\n",
        )
        ...
        write_plaintext_log(
            current.logFileName,
            "Action Taken on Comments: " + str(deletionModeFriendlyName) + "\n\n",
        )
        write_plaintext_log(
            current.logFileName,
            "Also Retrieved All Other Comments by Matched Authors: "
            + str(removeOtherAuthorComments)
            + "\n\n",
        )
...
def rewrite_log_file(current, logInfo, combinedCommentsDict=None):
    ...
    write_log_heading(
        ...
        filtersDict,
        afterExclude=True,
        combinedCommentsDict=combinedCommentsDict,
    )
