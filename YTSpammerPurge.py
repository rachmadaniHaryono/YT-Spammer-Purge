...
def main():
    ...
    if not os.path.isdir(resourceFolder):
        try:
            ...
            # Create readme
            with open(
                os.path.join(resourceFolder, "_What_Is_This_Folder.txt"), "w"
            ) as f:
                f.write(
                    "# This Resources folder is used to store resources required for the YT Spammer Purge program.\n"
                )
                f.write(
                    "# Note: If you had a previous spam_lists folder that was created in the same folder as \n"
                )
                f.write(
                    "# the .exe file, you can delete that old spam_lists folder. The resources folder is the \n"
                )
    ...
    spamListDict["Meta"]["VersionInfo"]["Path"] = os.path.join(
        spamListFolder, spamListDict["Meta"]["VersionInfo"]["FileName"]
    )  # Path to version included in packaged assets folder
    ...
    if not os.path.exists(spamListDict["Meta"]["VersionInfo"]["Path"]):
        files.copy_asset_file(
            spamListDict["Meta"]["VersionInfo"]["FileName"],
            spamListDict["Meta"]["VersionInfo"]["Path"],
        )
    ...
    try:
        if config["release_channel"] == "all":
            ...
            print(
                "Invalid value for 'release_channel' in config file. Must be 'All' or 'Stable'"
            )
    ...
    if config["auto_check_update"] == True:
        try:
            updateAvailable = files.check_for_update(
                version,
        ...
        if datetime.today() + timedelta(days=1) >= datetime.strptime(
            ...
            if datetime.today() > datetime.strptime(
                spamListDict["Meta"]["VersionInfo"]["LastChecked"], "%Y.%m.%d.%H.%M"
            ) + timedelta(days=1):
    ...
    for x, spamList in spamListDict["Lists"].items():
        spamList["FilterContents"] = files.ingest_list_file(
            spamList["Path"], keepCase=False
        )
    ...
    miscData.spamLists["spamAccountsList"] = spamListDict["Lists"]["Accounts"][
        "FilterContents"
    ]
    ...
    if not os.path.exists(whitelistPathWithName):
        with open(whitelistPathWithName, "a") as f:
            ...
            f.write(
                "# - Channels that were 'excluded' will also appear in this list.\n"
            )
            f.write(
        ...
        miscData.resources["Whitelist"]["WhitelistContents"] = files.ingest_list_file(
            whitelistPathWithName, keepCase=True
        )
    ...
    while confirmedCorrectLogin == False:
        ...
        CURRENTUSER = User(
            id=userInfo[0], name=userInfo[1], configMatch=userInfo[2]
        )  # Returns [channelID, channelTitle, configmatch]
        auth.CURRENTUSER = CURRENTUSER
        print(
            "\n    >  Currently logged in user: "
            + f"{F.LIGHTGREEN_EX}"
            + str(CURRENTUSER.name)
            + f"{S.R} (Channel ID: {F.LIGHTGREEN_EX}"
            + str(CURRENTUSER.id)
            + f"{S.R} )"
        )
    ...
    def primaryInstance(miscData):
        ...
        if updateAvailable != False:
            ...
            if updateAvailable == True:  # Stable update available
                ...
                print(
                    "> Note: Error during check for updates. Select 'Check For Updates' for details."
                )
        ...
        print(
            f"      3. Scan recent comments across your {F.LIGHTBLUE_EX}Entire Channel{S.R}"
        )
        print(
            f"      4. Scan a specific {F.LIGHTMAGENTA_EX}community post{S.R} (Experimental)"
        )
        print(
            f"      5. Scan {F.LIGHTMAGENTA_EX}recent community posts{S.R} for a channel (Experimental)"
        )
        print(
            f"\n--------------------------------- {F.YELLOW}Other Options{S.R} ----------------------------------"
        )
        print(
            f"      6. Create your own {F.LIGHTGREEN_EX}config file(s){S.R} to run the program with pre-set settings"
        )
        print(
            f"      7. Remove comments using a {F.LIGHTRED_EX}pre-existing list{S.R} or log file"
        )
            ...
            while confirm == False:
                ...
                while listNotEmpty == False or validVideoIDs == False:
                    if (
                        validConfigSetting == True
                        and config
                        and config["videos_to_scan"] != "ask"
                    ):
                        enteredVideosList = utils.string_to_list(
                            config["videos_to_scan"]
                        )
                    ...
                    videoListResult = (
                        []
                    )  # True/False, video ID, videoTitle, commentCount, channelID, channelTitle
                    for i in range(len(enteredVideosList)):
                        videoListResult.append([])
                        videosToScan.append({})
                        videoListResult[i] = validation.validate_video_id(
                            enteredVideosList[i]
                        )  # Sends link or video ID for isolation and validation
                ...
                for i in range(len(videoListResult)):  # Change this
                    ...
                    if allVideosMatchBool == True:
                        misMatchVidIndex = 0
                    if (
                        videosToScan[0]["channelOwnerID"]
                        != videosToScan[i]["channelOwnerID"]
                    ):
                        misMatchVidIndex += 1
                        if allVideosMatchBool == True:
                            print(
                            ...
                            print(
                                "  The following videos do not match the channel owner of the first video in the list: "
                            )
                        if misMatchVidIndex == 11 and len(enteredVideosList) > 10:
                            remainingCount = str(len(enteredVideosList) - 10)
                            userChoice = choice(
                                f"There are {remainingCount} more mis-matched videos, do you want to see the rest?"
                            )
                            if userChoice == False:
                                break
                                ...
                                return True  # Return to main menu
                        print(
                            f"  {misMatchVidIndex}. {str(videosToScan[i]['videoTitle'])}"
                        )
                        validConfigSetting = False
                ...
                if allVideosMatchBool == True:
                    ...
                    if config["skip_confirm_video"] == True:
                        ...
                        print(
                            "Total number of comments to scan: "
                            + str(miscData.totalCommentCount)
                        )
                        ...
                        confirm = choice(
                            "Is this video list correct?", bypass=validConfigSetting
                        )
                        if confirm == None:
                            return True  # Return to main menu
        ...
        # ============================================================================ RECENT VIDEOS ==========================================================================================================
            ...
            while validChannel == False:
                ...
                if config["channel_to_scan"] != "ask":
                    if config["channel_to_scan"] == "mine":
                        ...
                        (
                            ...
                            channelID,
                            channelTitle,
                        ) = validation.validate_channel_id(config["channel_to_scan"])
                ...
                print(
                    f"   > If scanning {F.YELLOW}your own channel{S.R}, just hit {F.LIGHTGREEN_EX}Enter{S.R}"
                )
                ...
                if inputtedChannel == "":
                    ...
                    (
                        ...
                        channelID,
                        channelTitle,
                    ) = validation.validate_channel_id(inputtedChannel)
            ...
            while validEntry == False or confirm == False:
                ...
                validConfigSetting = True
                if (
                    config["recent_videos_amount"] != "ask"
                    and validConfigSetting == True
                ):
                    ...
                    try:
                        ...
                        validConfigSetting = False
                        print(
                            "Invalid number entered in config file for recent_videos_amount"
                        )
                        numVideos = None
                else:
                    print(
                ...
                if validEntry == True:
                    ...
                    for i in range(len(videosToScan)):
                        if config["skip_confirm_video"] == False:
                            if i == 10 and len(videosToScan) > 11:
                                ...
                                userChoice = choice(
                                    f"There are {remainingCount} more recent videos, do you want to see the rest?"
                                )
                                if userChoice == False:
                                    break
                                elif userChoice == None:
                    ...
                    if (
                        config["skip_confirm_video"] == True
                        and validConfigSetting == True
                    ):
                        confirm = True
                    else:
                        ...
                        confirm = choice(
                            ...
                            bypass=config["skip_confirm_video"],
                        )
                        if confirm == None:
                            return True  # Return to main menu
            ...
            while validInteger == False:
                try:
                    if (
                        validConfigSetting == True
                        and config
                        and config["max_comments"] != "ask"
                    ):
                        maxScanNumber = int(config["max_comments"])
                    else:
                        maxScanNumber = input(
                            f"Enter the maximum {F.YELLOW}number of comments{S.R} to scan: "
                        )
        ...
        # ================================================================================ COMMUNITY POST =====================================================================================================
            ...
            print(
                "   > It is also much slower to retrieve comments, because it does not use the API"
            )
            confirm = False
            while confirm == False:
                ...
                (
                    ...
                    postOwnerID,
                    postOwnerUsername,
                ) = validation.validate_post_id(communityPostInput)
                if isValid == True:
                    ...
                    print(
                        "Problem interpreting the post information, please check the link or ID."
                    )
        ...
        # Recent Community Posts
            ...
            recentPostsListofDicts = community_downloader.fetch_recent_community_posts(
                channelID
            )
            ...
            while True:
                ...
                try:
                    ...
                    if numRecentPosts > len(recentPostsListofDicts):
                        print(
                            "Number entered is more than posts available. Will just scan all posts available."
                        )
        ...
        if (
            ...
            if filterMode == "Username":
                ...
                print(
                    "~~~ What do you want to scan names and comments for specifically? ~~~"
                )
            print(
                f" 1. A {F.CYAN}certain special character{S.R}, or set of multiple characters"
            )
        ...
        if filterMode == "ID":
            ...
            filterSettings = modes.prepare_filter_mode_smart(
                scanMode, config, miscData, sensitive=True
            )
            inputtedUsernameFilter = filterSettings[0]
            inputtedCommentTextFilter = filterSettings[0]

        elif filterSubMode == "chars":
            filterSettings = modes.prepare_filter_mode_chars(
                scanMode, filterMode, config
            )
        elif filterSubMode == "string":
            filterSettings = modes.prepare_filter_mode_strings(
                scanMode, filterMode, config
            )
        elif filterSubMode == "regex":
            filterSettings = modes.prepare_filter_mode_regex(
                scanMode, filterMode, config
            )
        ...
        if scanMode == "communityPost" or scanMode == "recentCommunityPosts":
            ...
            def scan_community_post(
                current,
                config,
                communityPostID,
                limit,
                postScanProgressDict=None,
                postText=None,
            ):
                authorKeyAllCommentsDict = {}
                allCommunityCommentsDict = get_community_comments(
                    communityPostID=communityPostID,
                    limit=limit,
                    postScanProgressDict=postScanProgressDict,
                    postText=postText,
                )
                ...
                for key, value in allCommunityCommentsDict.items():
                    ...
                    progressStats = (
                        f"[ {str(scannedCount)} / {str(retrievedCount)} ]".ljust(
                            15, " "
                        )
                        + f" ({percent:.2f}%)"
                    )
                    print(
                        f"  {progressStats}  -  Analyzing Comments For Spam ", end="\r"
                    )
                print(
                    "                                                                                        "
                )
            ...
            def scan_video(
                ...
                )  # Prints comment scan stats, updates on same line
                # After getting first page, if there are more pages, goes to get comments for next page
                while (
                    nextPageToken != "End"
                    and current.scannedCommentsCount < maxScanNumber
                ):
                    nextPageToken, currentVideoDict = operations.get_comments(
                        current,
                        filtersDict,
            ...
            if scanMode == "entireChannel":
                ...
                for video in videosToScan:
                    ...
                    status = scan_video(
                        ...
                        scanVideoID,
                        videosToScan=videosToScan,
                        currentVideoDict=currentVideoDict,
            ...
            if current.errorOccurred == False:
                operations.print_count_stats(
                    current, miscData, videosToScan, final=True
                )  # Prints comment scan stats, finalizes
            else:
        ...
        if config["enable_logging"] != "ask":
            ...
            if logSetting == True:
                ...
                bypass = False
                print(
                    "Error Code C-2: Invalid value for 'enable_logging' in config file:  "
                    + logSetting
                )
        ...
        if bypass == False:
            ...
            print(
                f"        (It even allows you to {F.LIGHTGREEN_EX}restore{S.R} deleted comments later)"
            )
        ...
        logFileContents, logMode = logging.print_comments(
        ...
        if (
            userNotChannelOwner == True
            and filterMode not in filterModesAllowedforNonOwners
        ):
            confirmDelete = False
            deletionEnabled = False
            print(
            ...
            response = input(
                "Press Enter to continue, or type 'x' to return to Main Menu..."
            )
            if response.lower() == "x":
                return True
        ...
        if config["skip_deletion"] == True:
            ...
            print(
                "Error Code C-3: Invalid value for 'skip_deletion' in config file. Must be 'True' or 'False'. Current Value:  "
                + str(config["skip_deletion"])
            )
        ...
        # User wants to automatically delete with no user intervention
            ...
            if userNotChannelOwner == True:
                ...
                config["removal_type"] == "reportspam"
                or config["removal_type"] == "heldforreview"
            ):
                if filterMode == "AutoSmart" or filterMode == "ID":
                    ...
                    print(
                        "Next time use one of those filter modes, or set 'delete_without_reviewing' to 'False'."
                    )
                    print(
                        "    > For this run, you will be asked to confirm removal of spam comments."
                    )
                    input("\nPress Enter to continue...")
                    confirmDelete = None
                    deletionEnabled = "Allowed"
                ...
                print(
                    "Next time, either set one of those removal types, or set 'delete_without_reviewing' to 'False'."
                )
                print(
                    "    > For this run, you will be asked to confirm removal of spam comments."
                )
            ...
            # Catch Invalid value
            print(
                "Error C-7: Invalid value for 'delete_without_reviewing' in config file. Must be 'True' or 'False':  "
                + config["delete_without_reviewing"]
            )
            input("\nPress Enter to exit...")
            sys.exit()
        ...
        if (
            returnToMenu == False
            and deletionEnabled != "Allowed"
            and deletionEnabled != True
        ):
            print(
        ...
        if confirmDelete == None and returnToMenu == False:
            ...
            while confirmDelete == None or confirmDelete.lower() not in validResponses:
                ...
                if userNotChannelOwner == True and moderator_mode == False:
                    print(
                        f"{F.GREEN}~~ Not Your Channel Mode: Only Reporting is Possible ~~{S.R}"
                    )
                if userNotChannelOwner == True and moderator_mode == True:
                    print(
                        f"{F.GREEN}~~ Moderator Mode: Reporting and Holding for Review is possible ~~{S.R}"
                    )
                ...
                if exclude == False:
                    ...
                    if (
                        userNotChannelOwner == False or moderator_mode == True
                    ) and current.errorOccurred == False:
                        print(
                ...
                if current.errorOccurred == False:
                    print(
                        f" > To {F.LIGHTCYAN_EX}report the comments for spam{S.R}, type '{F.LIGHTCYAN_EX}REPORT{S.R}'."
                    )
                if loggingEnabled:
                    print(
                        f" > To do nothing and {F.YELLOW}only log{S.R}, type '{F.YELLOW}NONE{S.R}'"
                    )
                else:
                    print(
                        f" > To do {F.YELLOW}nothing{S.R}, type '{F.YELLOW}NONE{S.R}'"
                    )

                if (
                    config["json_log"] == True
                    and config["json_extra_data"] == True
                    and loggingEnabled
                ):
                    print(
                ...
                if confirmDelete.lower() == "delete" and userNotChannelOwner == False:
                ...
                elif confirmDelete.lower() == "hold" and (
                    userNotChannelOwner == False or moderator_mode == True
                ):
                    deletionEnabled = True
                    deletionMode = "heldForReview"
                    ...
                    "exclude" in confirmDelete.lower()
                    or "only" in confirmDelete.lower()
                ):
        ...
        if returnToMenu == False:
            ...
            if (
                (
                    deletionMode == "rejected"
                    or deletionMode == "reportSpam"
                    or deletionMode == "heldForReview"
                )
                and deletionEnabled == True
                and current.errorOccurred == False
            ):
                ...
                if deletionMode == "rejected" or deletionMode == "heldForReview":
                    if config["remove_all_author_comments"] != "ask":
                        includeOtherAuthorComments = config[
        ...
        if includeOtherAuthorComments == True:
            current = operations.get_all_author_comments(
                current, config, miscData, current.allScannedCommentsDict
            )
        ...
        if loggingEnabled == True:
            ...
            if exclude == True:
                if logInfo["logMode"] == "rtf":
                    ...
                    logging.write_plaintext_log(
                        current.logFileName, str(plaintextFormattedExcludes)
                    )
            print("                                          ")
            ...
            if (
                ...
                if config["json_extra_data"] == True:
                    if current.errorOccurred == False:
                        jsonDataDict = logging.get_extra_json_data(
        ...
        if returnToMenu == False:
            if proceedWithDeletion == True:
                operations.delete_found_comments(
                    list(combinedCommentDict), banChoice, deletionMode
                )
            ...
            if config["auto_close"] == True:
                print(
                    "Deletion disabled due to error during scanning. Auto-close enabled in config. Exiting in 5 seconds..."
                )
                time.sleep(5)
                sys.exit()
            else:
            ...
            if config["auto_close"] == True:
                print(
                    "Deletion Cancelled. Auto-close enabled in config. Exiting in 5 seconds..."
                )
                time.sleep(5)
                sys.exit()
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if hx.status_code:
            ...
            print(
                f"If this keeps happening, consider posting a bug report on the GitHub issues page, and include the above error info."
            )
            print(f"Short Link: {F.YELLOW}TJoe.io/bug-report{S.R}")
            input("\nPress Enter to Exit...")
        else:
            print(
                f"{F.LIGHTRED_EX}Unknown Error - Code: Z-1{S.R} occurred. If this keeps happening, consider posting a bug report on the GitHub issues page, and include the above error info."
            )
            print(f"Short Link: {F.YELLOW}TJoe.io/bug-report{S.R}")
            input("\n Press Enter to Exit...")
        ...
        if "referenced before assignment" in str(ux):
            ...
            print(
                f"Please post a bug report on the GitHub issues page, and include the above error info."
            )
            print(f"Short Link: {F.YELLOW}TJoe.io/bug-report{S.R}")
            print(
                "    (In the mean time, try using a previous release of the program.)"
            )
            input("\n Press Enter to Exit...")
        else:
            traceback.print_exc()
            ...
            print(
                "consider posting a bug report on the GitHub issues page, and include the above error info."
            )
            print(f"Short Link: {F.YELLOW}TJoe.io/bug-report{S.R}")
            input("\n Press Enter to Exit...")
        ...
        if "config" in str(kx):
            ...
            print(
                "Are you using an outdated version of the config file? Try re-creating the config file to get the latest version."
            )
            print(
                f"{F.LIGHTYELLOW_EX}If that doesn't work{S.R}, consider posting a {F.LIGHTYELLOW_EX}bug report{S.R} on the GitHub issues page, and include the above error info."
            )
        else:
            print(
                f"{F.RED}Unknown Error - Code: X-4{S.R} occurred. This is {F.YELLOW}probably my fault{S.R},"
            )
            print(
                f"please a {F.LIGHTYELLOW_EX}bug report{S.R} on the GitHub issues page, and include the above error info."
            )
        print(f"Short Link: {F.YELLOW}TJoe.io/bug-report{S.R}")
        input("\n Press Enter to Exit...")
    except TypeError:
        ...
        print("------------------------------------------------")
        print(
            f"{F.RED}Unknown Error - Code: X-5{S.R} occurred. This is {F.YELLOW}probably my fault{S.R},"
        )
        print(
            f"please a {F.LIGHTYELLOW_EX}bug report{S.R} on the GitHub issues page, and include the above error info."
        )
